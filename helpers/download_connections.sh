...
soln=$(grep 'What is the answer to' $html | grep -o '</strong>: [A-Z, ]\+</p>' | gsed 's|</\?[a-z]\+>:\? \?||g' | gsed '1s/^/**Yellow:** ||/' | gsed '2s/^/**Green:** ||/' | gsed '3s/^/**Blue:** ||/' | gsed '4s/^/**Purple:** ||/' | gsed 's/, /||   ||/g' | gsed 's/$/||/' | gsed 's/^/- /')
