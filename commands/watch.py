...
class Watch(BaseCommand):
    ...
    def __init__(self):
        ...
        description = "Manages watchlists"
    ...
    async def handle(self, params, message, client):
        ...
        savefile = "/Users/dianalin/mocking-spongebob/files/watchlist.json"
        if len(params) > 0: 
            ...
            if os.path.exists(savefile) == True:
                ...
                watchlist = {}
#                 jsonfile = json.dumps(watchlist, indent = 4)
#                 with open(savefile, "w") as f:
#                     f.write(jsonfile)

            # will be called watchlist
            if subcommand not in ["add", "remove", "clear", "list", "delete", "edit", "append", "random"]:
                ...
                if sublist not in ["all"] + list(watchlist.keys()):
                    # create the list
                ...
                if subcommand == "add":
                    ...
                    if sublist not in list(watchlist.keys()):
                        if sublist == "":
                        ...
                        if item != "":
                            ...
                            try:
                                ...
                                index = int(string[0])
                                if index not in range(1, len(watchlist[sublist])+1):
                                    await message.channel.send("Unrecognized index: *{index}*")
                                else:
                                    if subcommand == "edit":
                                        new_item = " ".join(string[1:])
                                        old_item = watchlist[sublist][index-1] 
                                        watchlist[sublist][index-1] = new_item
                                        ...
                                        old_item = watchlist[sublist][index-1] 
                                        new_item = " ".join(string[1:])
                                        watchlist[sublist][index-1] = old_item + " " + new_item
                            ...
                            for i in indices:
                                try:
                                    ...
                                    if index not in range(1, len(copy)+1):
                                        ...
                                        idx = watchlist[sublist].index(item) # get new index from new list using original item
                                        temp = watchlist[sublist].pop(idx) # use the new index to pop 
                                        removed.append(item)
                                        # await message.channel.send(f"Removed *{item}* from **{sublist}**.")
                                except ValueError as ve:
                                    if i not in watchlist[sublist]:
                                        await message.channel.send(f"Unrecognized item: *{i}*")
                                        continue
                                    else:
                                        # remove using name
                                        watchlist[sublist].remove(i)
                                        removed.append(i)
                                        # await message.channel.send(f"Removed *{item}* from **{sublist}**.")
                            text = ", ".join(removed)
                    ...
                    if sublist == "all":
                        watchlist = {}
                        await message.channel.send("Deleted all watchlists.")
                        jsonfile = json.dumps(watchlist, indent = 4)
                    ...
                    if sublist == "all":
                        for i in list(watchlist.keys()):
                            watchlist[i] = []
                        jsonfile = json.dumps(watchlist, indent = 4)
                        with open(savefile, "w") as f:
                            f.write(jsonfile)
                    elif sublist not in list(watchlist.keys()):
                        if sublist != "":
                            await message.channel.send(f"Unrecognized list: **{sublist}**")
                        else:
                            await message.channel.send(f"Missing list argument.")
                    ...
                    if sublist not in list(watchlist.keys()):
                        ...
                        chosen = random.choice(watchlist[sublist])
                    ...
                    if sublist == "all" or sublist == "":
                        if len(watchlist) > 0:
                            ...
                            for i in list(watchlist.keys()):
                                items = []
                                if len(watchlist[i]) > 0:
                                    for index, j in enumerate(watchlist[i]):
                                        idx = index+1
                                        items.append(f"\t{idx : >2}. {j}")
                                    temp = [ f":clapper: **{i}**" ] + items
                                    chunk = '\n'.join(temp)
                                    msg.append(chunk)
                                else:
                                    chunk = f":clapper: **{i}**\n\t*No watchlist.*"
                            ...
                            await message.channel.send("You have no watchlists.")
                    elif sublist not in list(watchlist.keys()):
                        if sublist != "":
                            await message.channel.send(f"Unrecognized list: **{sublist}**")
                        else:
                        ...
                        msg = [ f":clapper: **{sublist}**" ]
                        items = []
                        for index, i in enumerate(watchlist[sublist]):
                            idx = index+1
